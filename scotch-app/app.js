...
function renderRound() {
  ...
  root.appendChild(h('div', { class: 'hole-header' },
    ...
    h('div', { style: 'font-size:13px;font-weight:700;color:rgba(255,255,255,0.9);margin-top:6px;display:flex;gap:14px;justify-content:center;' },
      h('span', null, `Top: ${pressSlashStr(result.mode === '5man' ? result.game1.top : result.top, holesBefore)}`),
      h('span', null, `Mid: ${tallyStr(beforeMid)}`),
      h('span', null, `Bot: ${pressSlashStr(result.mode === '5man' ? result.game1.bottom : result.bottom, holesBefore)}`)
    ),
