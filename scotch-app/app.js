...
function renderLiveView() {
  ...
  const chatCard = h('div', { class: 'card' },
    ...
    !state.liveChatName ? h('div', { style: 'margin-bottom:10px;' },
      h('div', { class: 'field' },
      ...
      h('div', { style: 'font-size:11px;color:var(--muted);' }, 'Press Enter to join chat')
    ) : null,
    // Messages
    h('div', {
      id: 'chat-messages',
    ...
    // Input row
    state.liveChatName ? h('div', { style: 'display:flex;gap:8px;' },
      h('input', {
        type: 'text',
        id: 'chat-input',
        ...
        onkeydown: e => {
          if (e.key === 'Enter' && state.liveChatInput.trim() && state.liveChatHandle) {
      ...
      h('button', { class: 'btn', style: 'padding:8px 16px;', onclick: () => {
        if (state.liveChatInput.trim() && state.liveChatHandle) {
          const msg = { name: state.liveChatName, text: state.liveChatInput.trim(), ts: Date.now() };
          state.liveChatHandle.send(msg);
          state.liveChatMessages.push(msg);
          if (state.liveChatMessages.length > 100) state.liveChatMessages.shift();
