...
function renderRound() {
  ...
  function sumTopHoles(topGame, holes) {
    let a = 0, b = 0;
    const allSegs = [...topGame.segments, ...topGame.presses];
    for (const seg of allSegs) {
      for (const p of seg.points) { if (holes.includes(p.h)) { a += p.a; b += p.b; } }
    }
    return { a, b };
  }
  function sumBottomHoles(bottomGame, holes) {
    let a = 0, b = 0;
    const allSegs = [...bottomGame.segments, ...bottomGame.presses];
    for (const seg of allSegs) {
      for (const p of seg.points) { if (holes.includes(p.h)) { a += p.a; b += p.b; } }
    }
    return { a, b };
  ...
  const beforeTop = result.mode === '5man'
    ? { a: sumTopHoles(result.game1.top, holesBefore).a + sumTopHoles(result.game2.top, holesBefore).a,
        b: sumTopHoles(result.game1.top, holesBefore).b + sumTopHoles(result.game2.top, holesBefore).b }
    : sumTopHoles(result.top, holesBefore);
  const beforeBot = result.mode === '5man'
    ? { a: sumBottomHoles(result.game1.bottom, holesBefore).a + sumBottomHoles(result.game2.bottom, holesBefore).a,
        b: sumBottomHoles(result.game1.bottom, holesBefore).b + sumBottomHoles(result.game2.bottom, holesBefore).b }
    : sumBottomHoles(result.bottom, holesBefore);
  ...
  const thruTop = result.mode === '5man'
    ? { a: sumTopHoles(result.game1.top, holesThrough).a + sumTopHoles(result.game2.top, holesThrough).a,
        b: sumTopHoles(result.game1.top, holesThrough).b + sumTopHoles(result.game2.top, holesThrough).b }
    : sumTopHoles(result.top, holesThrough);
  const thruBot = result.mode === '5man'
    ? { a: sumBottomHoles(result.game1.bottom, holesThrough).a + sumBottomHoles(result.game2.bottom, holesThrough).a,
        b: sumBottomHoles(result.game1.bottom, holesThrough).b + sumBottomHoles(result.game2.bottom, holesThrough).b }
    : sumBottomHoles(result.bottom, holesThrough);
  ...
  // Hole header — shows tally BEFORE this hole
  root.appendChild(h('div', { class: 'hole-header' },
    h('div', { class: 'hole-number' }, String(hIdx + 1)),
    h('div', { style: 'font-size:12px;font-weight:600;color:rgba(255,255,255,0.8);margin-top:4px;display:flex;gap:12px;justify-content:center;' },
      h('span', null, `Mid: ${beforeMid.a}–${beforeMid.b}`),
      h('span', null, `Top: ${beforeTop.a}–${beforeTop.b}`),
      h('span', null, `Bot: ${beforeBot.a}–${beforeBot.b}`)
    ),
  ...
  root.appendChild(h('div', { style: 'text-align:center;padding:10px 14px;margin:0 14px;background:var(--card);border-radius:var(--radius);border:1px solid var(--border-light);font-size:12px;font-weight:700;display:flex;justify-content:space-around;' },
    h('div', null,
      h('div', { style: 'font-size:9px;text-transform:uppercase;color:var(--muted);letter-spacing:1px;' }, 'Middle'),
      h('div', { style: `color:${tallyClr(thruMid)};` }, `${thruMid.a} – ${thruMid.b}`)
    ),
    h('div', null,
      h('div', { style: 'font-size:9px;text-transform:uppercase;color:var(--muted);letter-spacing:1px;' }, 'Top'),
      h('div', { style: `color:${tallyClr(thruTop)};` }, `${thruTop.a} – ${thruTop.b}`)
    ),
    h('div', null,
      h('div', { style: 'font-size:9px;text-transform:uppercase;color:var(--muted);letter-spacing:1px;' }, 'Bottom'),
      h('div', { style: `color:${tallyClr(thruBot)};` }, `${thruBot.a} – ${thruBot.b}`)
    )
  ));
