      ...
      h('tbody', null,
        ...COURSES.map(c => {
          ...
          if (isEditing) {
            ...
            if (!c._editTees) c._editTees = (c.tees || []).map(t => ({ ...t, si: t.si ? [...t.si] : null }));
            ...
            for (let hole = 0; hole < 18; hole++) {
              ...
              scorecardRows.push(h('tr', null,
                ...
                h('td', null, h('input', { type: 'number', value: editSi[hole], min: 1, max: 18, style: 'width:45px;text-align:center;padding:4px;',
                  oninput: e => { editSi[hole] = parseInt(e.target.value) || 1; } })),
                ...editTees.map(t => {
                  if (!t.si) t.si = [...editSi]; // seed from default
                  return h('td', null, h('input', { type: 'number', value: t.si[hole], min: 1, max: 18, style: 'width:45px;text-align:center;padding:4px;',
                    oninput: e => { t.si[hole] = parseInt(e.target.value) || 1; } }));
                })
              ));
            }
            // Totals row
            ...
            return h('tr', { style: 'background:#fffdf5;vertical-align:top;' },
              h('td', { colspan: '4' },
                ...
                h('button', { class: 'btn sm secondary', style: 'margin-bottom:12px;', onclick: () => { editTees.push({ name: '', rating: null, si: [...editSi] }); render(); } }, '+ Add Tee'),
