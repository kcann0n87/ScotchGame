...
const COURSES = [
  ...
  { name: 'The Wanderers Club',
    ...
    tees: [
      { name: 'Black', rating: 74.2 }, { name: 'Blue', rating: 72.0 }, { name: 'Green', rating: 70.1 }, { name: 'Gold', rating: 67.3 }
    ]}
];
...
async function loadAllData() {
  ...
  // Auto-sync any preset courses that aren't in the DB yet
  const dbNames = new Set(allCourses.map(c => c.name));
  for (const c of COURSES) {
    if (!dbNames.has(c.name) && c.pars && c.pars.length > 0) {
      db.from('courses').insert({
        ...
        if (!error) console.log('Auto-synced course to DB:', c.name);
        else console.warn('Failed to sync course:', c.name, error);
      });
    }
  }
  render();
...
function renderPlayers(app) {
  ...
  // Unlinked players — round_players rows with no user_id
  const unlinked = allRoundPlayers.filter(rp => !rp.user_id && rp.display_name);
  // Group by name to avoid duplicates
  const unlinkedByName = {};
  for (const rp of unlinked) {
    const key = rp.display_name.toLowerCase().trim();
    if (!unlinkedByName[key]) unlinkedByName[key] = { name: rp.display_name, rows: [] };
    unlinkedByName[key].rows.push(rp);
  }
  const unlinkedNames = Object.values(unlinkedByName);

  if (unlinkedNames.length > 0) {
    app.appendChild(h('div', { class: 'card' },
      h('h2', null, `Unlinked Players (${unlinkedNames.length})`),
      h('div', { style: 'font-size:12px;color:var(--muted);margin-bottom:12px;' },
        'These names appeared in rounds but are not linked to any account. Assign them to a profile so their rounds count toward handicaps and the ledger.'),
      h('table', null,
        h('thead', null, h('tr', null,
          h('th', { style: 'text-align:left;' }, 'Name'),
          h('th', null, 'Rounds'),
          h('th', null, 'Link to Profile'),
          h('th', null, '')
        )),
        h('tbody', null,
          ...unlinkedNames.map(un => {
            return h('tr', null,
              h('td', { style: 'font-weight:600;' }, un.name),
              h('td', null, String(un.rows.length)),
              h('td', null,
                h('select', { id: `link-${un.name}`, style: 'width:100%;' },
                  h('option', { value: '' }, '— Select Profile —'),
                  ...allProfiles.map(p => h('option', { value: p.id }, `${p.display_name} (${p.email || 'no email'})`))
                )
              ),
              h('td', null,
                h('button', { class: 'btn sm', onclick: async () => {
                  const sel = document.getElementById(`link-${un.name}`);
                  const profileId = sel?.value;
                  if (!profileId) { alert('Select a profile first'); return; }
                  // Update all round_players rows with this name to point to the selected profile
                  let updated = 0;
                  for (const rp of un.rows) {
                    await db.from('round_players').update({ user_id: profileId }).eq('id', rp.id);
                    updated++;
                  }
                  alert(`Linked ${updated} round(s) for "${un.name}" to the selected profile.`);
...
function renderDashboard(app) {
  ...
  if (pendingRounds.length > 0) {
    for (const r of pendingRounds) {
      ...
      const isExpanded = selectedPlayerId === r.id;

      // Extract player scores and amounts from the round data
      const roundData = r.data || {};
      const allRoundPlayersData = [...(roundData.teamA || []), ...(roundData.teamB || [])];

      app.appendChild(h('div', { class: 'card', style: 'margin-top:16px;border:2px solid var(--gold);' },
        h('h2', null, 'Pending Round'),
        // Header row
        h('div', { style: 'display:flex;justify-content:space-between;align-items:center;margin-bottom:12px;' },
          h('div', null,
            h('div', { style: 'font-weight:700;font-size:15px;' }, r.course_name),
            h('div', { style: 'font-size:12px;color:var(--muted);' },
              `${new Date(r.played_at).toLocaleDateString()} · Scored by ${scorer?.display_name || '—'}`)
          ),
          h('div', { style: 'display:flex;gap:4px;' },
            ...
            h('button', { class: 'btn sm danger', onclick: async () => {
              if (!confirm('Reject this round?')) return;
        ...
        // Player scores and amounts
        h('table', null,
          h('thead', null, h('tr', null,
            h('th', { style: 'text-align:left;' }, 'Player'),
            h('th', null, 'Team'),
            h('th', null, 'Gross'),
            h('th', null, 'Win/Loss')
          )),
          h('tbody', null,
            ...rps.map(rp => {
              // Find this player's scores in the round data
              const playerData = allRoundPlayersData.find(p => p.name === rp.display_name);
              const scores = playerData?.scores || [];
              const gross = scores.filter(s => s != null).reduce((s, v) => s + v, 0);
              const holesPlayed = scores.filter(s => s != null).length;
              const amt = rp.final_amount || 0;
              const amtColor = amt > 0 ? 'positive' : amt < 0 ? 'negative' : 'zero';
              const amtStr = amt === 0 ? '$0' : amt > 0 ? `+$${amt}` : `-$${Math.abs(amt)}`;
              return h('tr', null,
                h('td', { style: 'font-weight:600;text-align:left;' }, rp.display_name),
                h('td', null, rp.team),
                h('td', null, holesPlayed > 0 ? String(gross) : '—'),
                h('td', { class: amtColor, style: 'font-weight:700;' }, amtStr)
              );
            })
          )
        )
          ...
          h('div', { style: 'display:flex;gap:10px;' },
            h('div', { class: 'field', style: 'flex:2;' },
              ...
              h('select', { style: 'width:100%;', onchange: e => {
                ...
                // Auto-set tee to first available
                const tees = getCourseTees(manualCourse);
                if (tees.length > 0) {
                  manualTee = tees[0].name;
                  const r = getTeeRating(manualCourse, manualTee);
                  if (r) manualRating = String(r);
                }
                render();
              } },
                h('option', { value: '' }, '-- Select Course --'),
                ...COURSES.map(c => h('option', { value: c.name, ...(manualCourse === c.name ? { selected: 'selected' } : {}) }, c.name))
              )
            ),
            h('div', { class: 'field', style: 'flex:1;' },
              h('label', null, 'Tee'),
              (() => {
                const tees = getCourseTees(manualCourse);
                if (tees.length > 0) {
                  return h('select', { style: 'width:100%;', onchange: e => {
                    manualTee = e.target.value;
                    const r = getTeeRating(manualCourse, manualTee);
                    manualRating = r ? String(r) : '';
                    render();
                  } },
                    ...tees.map(t => h('option', { value: t.name, ...(manualTee === t.name ? { selected: 'selected' } : {}) }, t.name))
                  );
                }
                return h('input', { type: 'text', value: manualTee, placeholder: 'Blue', oninput: e => { manualTee = e.target.value; } });
              })()
            )
          ),
          h('div', { style: 'display:flex;gap:10px;' },
            h('div', { class: 'field', style: 'flex:1;' },
              ...
              h('input', { type: 'number', step: '0.1', value: manualRating, placeholder: '72.5',
                style: manualRating ? 'font-weight:700;' : '',
                oninput: e => { manualRating = e.target.value; } })
            ),
            h('div', { class: 'field', style: 'flex:1;' },
