          ...
          if (isEditing) {
            return h('tr', { style: 'background:#fffdf5;vertical-align:top;' },
              h('td', { colspan: '4' },
                h('div', { class: 'field' },
                  h('label', null, 'Course Name'),
                  h('input', { type: 'text', value: c.name, oninput: e => { c._editName = e.target.value; } })
                ),
                h('h3', null, 'Tees'),
                ...(c._editTees || c.tees || []).map((t, i) =>
                  h('div', { style: 'display:flex;gap:8px;align-items:center;margin-bottom:6px;' },
                    h('input', { type: 'text', value: t.name, placeholder: 'Tee', style: 'flex:1;', oninput: e => { t.name = e.target.value; } }),
                    ...
                    h('button', { class: 'btn sm danger', style: 'width:auto;', onclick: () => {
                      if (!c._editTees) c._editTees = [...(c.tees || [])];
                      c._editTees.splice(i, 1);
                      render();
                    } }, 'x')
                  )
                ),
                h('button', { class: 'btn sm secondary', style: 'margin-bottom:8px;', onclick: () => {
                  if (!c._editTees) c._editTees = [...(c.tees || [])];
                  c._editTees.push({ name: '', rating: null });
                  render();
                } }, '+ Add Tee'),
                h('div', { style: 'display:flex;gap:8px;margin-top:8px;' },
                  h('button', { class: 'btn sm', onclick: async () => {
                    const name = (c._editName || c.name).trim();
                    const tees = (c._editTees || c.tees || []).filter(t => t.name?.trim()).map(t => ({ name: t.name.trim(), rating: t.rating }));
                    if (c.dbId) {
                      await db.from('courses').update({ name, tees, updated_at: new Date().toISOString() }).eq('id', c.dbId);
                    } else {
                      await db.from('courses').insert({ name, tees, created_by: user.id });
                    }
                    editingCourseId = null;
                    allCourses = null;
                    await loadAllData();
                  } }, 'Save'),
                  h('button', { class: 'btn sm secondary', onclick: () => { editingCourseId = null; delete c._editTees; delete c._editName; render(); } }, 'Cancel')
                )
              )
            );
          }
